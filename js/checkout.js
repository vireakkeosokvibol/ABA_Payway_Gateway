  ...
  var wc_checkout_form = {
    ...
    payment_method_selected: function (e) {
      ...
      if (jQuery(".payment_methods input.input-radio").length > 1) {
        var target_payment_box = jQuery(
            "div.payment_box." + jQuery(this).attr("ID")
          ),
    ...
    maybe_update_checkout: function () {
      ...
      if (jQuery(wc_checkout_form.dirtyInput).length) {
        var $required_inputs = jQuery(wc_checkout_form.dirtyInput)
          .closest("div")
      ...
      wc_checkout_form.xhr = jQuery.ajax({
        ...
        success: function (data) {
          ...
          jQuery(".payment_box :input").each(function () {
            ...
            if (ID) {
              if (
                jQuery.inArray(jQuery(this).attr("type"), [
                  "checkbox",
                  "radio",
                ]) !== -1
              ) {
    ...
    submit: function () {
      ...
      if (
        ...
        if (
          selected_payment_option == "aba_payway_aim" ||
          selected_payment_option == "aba_payway_aim_card"
        ) {
          ...
          if (jQuery("#a_choosen_method").length == 0) {
            $input = jQuery(
              '<input type="hidden" name="aba_choosen_method" id="a_choosen_method"/>'
            ).val(choosen_method);
            $form.append($input);
          } else {
            ...
            try {
              if ("success" === result.result) {
                if (
                  ...
                  jQuery("#aba_main_modal [name='tran_id']").val(
                    result.order_id
                  );
                  jQuery("#aba_main_modal [name='items']").val(result.items);
                  if (selected_payment_option == "aba_payway_aim") {
                    jQuery("#aba_main_modal [name='payment_option']").val(
                      "abapay"
                    );
                  } else {
                    jQuery("#aba_main_modal [name='payment_option']").val(
